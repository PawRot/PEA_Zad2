...
int main(int argc, char **argv) {
    if (argc > 1 && std::string(argv[1]) == "testMode"){ // enter test mode
        ...
        if (argc < 3){ // testing requires second argument selecting which file will be tested
            std::cout << "No second argument provided" << std::endl;
            exit(0);
        }

        tester::run_tests(std::string(argv[2]));
    ...
    int choice = -1;
    string input;
    bool dataLoaded = false; // flag indicating if data is loaded
    bool stopCriterionSet = false; // flag indicating if stop criterion is set
    int stopCriterion = 0; // stop criterion in seconds
    bool pathLoaded = false; // flag indicating if path is loaded
    long double tempChangefactor = 0.99999; // temperature change factor
    vector<vector<int>> testData; // loaded test data
    vector<int> path; // loaded path
...
void startSimulatedAnnealing(const vector<vector<int>>&testData, vector<int>&path, const long double &tempChangeFactor, const int &stopCriterion, bool &pathLoaded){
    ...
    std::cout << std::endl;
    greedy greedy(testData); // create greedy object
    const auto greedyResult = greedy.findShortestPath(); // find shortest path using greedy algorithm
    ...
    simulatedAnnealing simulatedAnnealing(testData, tempChangeFactor, stopCriterion, greedyResult); // create simulated annealing object

    const auto result = simulatedAnnealing.simulatedAnnealingAlgorithm(); // start simulated annealing algorithm
